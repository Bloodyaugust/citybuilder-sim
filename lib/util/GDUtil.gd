...
static func get_tile_from_offset_global(position: Vector2, offset: Vector2, tilemap: TileMap) -> Vector2i:
  return get_tile_from_global_position(position + (offset * Vector2(tilemap.tile_set.tile_size)), tilemap)
